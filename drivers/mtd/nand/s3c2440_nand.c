...
#define S3C2440_ADDR_NCLE 0x8		/* CLE : Command Latch Enable */
#define S3C2440_ADDR_NALE 0xC		/* ALE : Address Latch Enable */
...
static void s3c2440_hwcontrol(struct mtd_info *mtd, int cmd, unsigned int ctrl)
{
	...
	if (ctrl & NAND_CTRL_CHANGE) {
		...
		if (ctrl & NAND_NCE)	/* Enable Chip Select - Force nFCE to low */
			writel(readl(&nand->nfcont) & ~S3C2440_NFCONT_nFCE,
			       &nand->nfcont);
		else	/* Disable Chip Select - Force nFCE to high */
			writel(readl(&nand->nfcont) | S3C2440_NFCONT_nFCE,
			       &nand->nfcont);
	}
...
void s3c2440_nand_enable_hwecc(struct mtd_info *mtd, int mode)
{
	...
	writel(readl(&nand->nfcont) | S3C2440_NFCONT_INITECC & (~S3C2440_NFCONT_MECCL)
			 ,&nand->nfcont);
}



/* Need to modify to work properly */
...
int board_nand_init(struct nand_chip *nand)
{
	...
	/* CLKCON bit[4] - NAND flash controller Enable */
	...
	cont = S3C2440_NFCONT_EN;

#ifdef CONFIG_S3C2440_NAND_HWECC
	cont |= S3C2440_NFCONT_INITECC;
	cont |= S3C2440_NFCONT_SECCL;
	cont |= S3C2440_NFCONT_MECCL;
#endif

	cont |= S3C2440_NFCONT_nFCE;	
